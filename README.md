...
Локально пройдены typecheck, 8 тестов, production build, HTTP-сценарий поиска/чата/подтверждения и браузерный сценарий в Edge на desktop и ширине 390 px. Скриншоты в `docs/`. `check-browser.mjs` использует установленный Microsoft Edge по стандартному Windows-пути. Live EKT API и реальный AI tool call пока не проверены: учётные данные и ключ не были доступны в рабочем окружении. После настройки `.env` нужно отдельно проверить их на свежих товарах и обновить [сценарий демонстрации](docs/demo.md).
...
- Пользовательское изменение/удаление позиции из сохранённой корзины пока не реализовано.
