...
const examples = {
  demo: ["DEMO-AV16", "DEMO-AV16-OLD", "Условия доставки в Алматы", "Нужен автомат на 16 А"],
  live: ["310100086_", "200300265_", "Условия доставки в Алматы", "515291"],
};
...
function AnalogCard({ item, city, onPrepare }: { item: NonNullable<ChatReply["analogs"]>[number]; city?: string | null; onPrepare: (id: number, quantity: number) => void }) {
  const stock = availability(item.product, city || undefined);
  return <div className="analog">
    <strong>Возможная замена: {item.product.name}</strong>
    <p>Артикул {item.product.article} · {formatMoney(item.product.price!)} · {stock.label}: {stock.quantity} шт.</p>
    <p>Совпадает: {item.matches.join(", ")}</p>
    {item.differences.length > 0 && <p>Различия: {item.differences.join(", ")}</p>}
    {item.caveats.map(caveat => <p className="warning" key={caveat}>{caveat}</p>)}
    {item.product.url && <a href={item.product.url} target="_blank" rel="noreferrer">Карточка EKT ↗</a>}
    <button className="text-button" onClick={() => onPrepare(item.product.id, 1)}>Предложить 1 шт.</button>
  </div>;
}
