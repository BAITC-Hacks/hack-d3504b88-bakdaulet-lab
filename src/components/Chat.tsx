...
function ProductCard({ product, onPrepare }: { product: Product; onPrepare: (id: number, quantity: number) => void }) {
  ...
  return <article className="product-card"><div className="product-main"><div className="product-image">{product.image ? <img src={product.image} alt="" /> : <span>ЕКТ</span>}</div><div><p className="card-kicker">АРТИКУЛ {product.article || "не указан"}</p><h3>{product.name}</h3><p className="muted">{product.supplierArticle ? `Артикул поставщика: ${product.supplierArticle}` : ""}</p><p className="product-price">{price}</p></div></div>
    <div className="stock-row"><span className={product.quantity && product.quantity > 0 ? "stock-chip" : "stock-chip muted-chip"}>{product.quantity === null ? "Наличие неизвестно" : product.quantity > 0 ? `Общий остаток: ${product.quantity} шт.` : "Нет в наличии"}</span><span className="muted">Проверено {new Date(product.fetchedAt).toLocaleString("ru-RU")}</span></div>
    ...
    <div className="card-actions">{product.url && <a href={product.url} target="_blank" rel="noreferrer">Карточка EKT ↗</a>}{product.quantity !== null && product.quantity > 0 && product.price !== null && <><label className="quantity-label">Кол-во <input type="number" min="1" step="1" value={quantity} onChange={e => setQuantity(Number(e.target.value))}/></label><button type="button" className="small-primary" onClick={() => onPrepare(product.id, quantity)}>Подготовить предложение</button></>}</div>
  </article>;
}
